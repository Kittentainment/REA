...
class WarningInfo is export {
    ...
    method toString(:$symbolForSevereAnswers = '!', :$lineIndent = "\t") returns Str {
        given $!warning {
            when INTRO_MISMATCH {
            ...
            when ANSWER_MISMATCH_ERROR {
                return $lineIndent ~ "$symbolForSevereAnswers Answer Mismatch: The answers don't match the ones from the exam file on question $!questionNumber."
            }
        }
#        return $!warning.Str;
    }
...
sub evaluateFilledOutFileExactly(:$parsedMasterFile, :$parsedFilledOutFile) returns TestResult {
    ...
    for ^$parsedMasterFile.QACombos -> $QAComboIndex {
        ...
        unless ($parsedFilledOutFile.QACombos[$QAComboIndex]) {
            ...
            @warnings.append(WarningInfo.new(warning => QUESTION_COUNT_ERROR, questionNumber => $QAComboIndex, :$actualQuestionCount, :$expectedQuestionCount));
            last;
        }
